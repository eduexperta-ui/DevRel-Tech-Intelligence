          ...
          <div className="overflow-x-auto rounded-2xl border border-neutral-200">
            <table className="min-w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-neutral-100">
                {(dashboardData.companyPosts ?? []).map((post, index) => (
      ...
      return (
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            {(dashboardData.implementationPatterns ?? []).map(
              (item, index) => (
                <article
      ...
      return (
        <section className="space-y-4">
          ...
          <div className="space-y-4">
            {(dashboardData.devexSignals ?? []).map((item, index) => (
              <article
    ...
    case "global-engineering":
      return (
        <section className="space-y-4">
          <div>
            <p className="text-xs font-bold uppercase tracking-wider text-neutral-500">
              Official and community signals
            </p>
            <h3 className="mt-1 text-xl font-black text-neutral-900">
              글로벌 엔지니어링 시그널
            </h3>
          ...
          <div className="space-y-3">
            {(dashboardData.globalSignals ?? []).map((item, index) => (
              <article
